...
- **Newton FBA:** `newton/_src/solvers/fba/solver_fba.py:638-658` — at step entry resets `_lam_unilateral_persistent`, `_lam_coulomb_persistent`, `_omega_unilateral_persistent`, `_omega_coulomb_persistent` to zero, sized to current `M`. The GPU drivers `_solve_nsn_unilateral_gpu`/`_solve_nsn_coulomb_gpu` (`solver_fba.py:1700-2100`, post Step-13 NSN GPU port) accept `lam_init` and `omega_init` host arguments, upload them to the device-resident NSN inner buffers, and accumulate `lam += dlam` per FB-Newton iter on-GPU via `update_lambda` (kernels.py NSN section).  The host fp64 numpy arrays remain the source of truth between PD outer iters — the GPU buffers are read back at the end of each `_solve_nsn_*_gpu` call.
- **Verdict:** **DIVERGE-accidental**.
...
- **Newton FBA:** Following the Step 1-13 NSN GPU port (`docs/superpowers/plans/2026-05-17-fba-nsn-gpu-port.md`) the inner solve is GPU-only.  Reference files:
  - `newton/_src/solvers/fba/kernels.py:3133-3600` — `@wp.func fb_unilateral_row` / `fb_frictional_row` (Step 2/3), `build_nsn_schur_kernel` (Step 4), `update_penetration_unilateral_kernel` (Step 7), `assemble_nsn_rhs_kernel` (Step 8), `update_lambda` + `clamp_lambda` (Step 9/10), and `_residual_unilateral`/`_residual_friction` (Step 6, currently host fallback — see Step 13 follow-up).
  - `newton/_src/solvers/fba/nsn_pcr_solver.py` — `NSNPCRSolver` class: ported `CUDADenseJacobiPCRSolver` from RealSim with batched per-iter `‖r‖²` reduction (Step 5).
  - `newton/_src/solvers/fba/solver_fba.py:1700-2100` — `_solve_nsn_unilateral_gpu` / `_solve_nsn_coulomb_gpu`: Python drivers that orchestrate the per-FB-Newton-iter dispatch.  Same dt² scaling and Schur-row algebra as the legacy numpy reference (now `_solve_nsn_unilateral` / `_solve_nsn_coulomb`, retained only as a parity oracle for `test_fba_nsn_gpu_driver`).  Unilateral row, frictional row, Schur LHS, penetration, and RHS formulas are unchanged — see the **algebra blocks below** for the dt²/_delasus argument that establishes 1:1 equivalence with RealSim's path; the only difference is that all arrays live on the device.
  ...
  - Schur LHS: `A_schur = (ω·ω^T) * W + diag(compliance)` — exactly RealSim's `ω·W·ω + diag(compliance)` once you note both sides build a rank-1 outer-product as `(ω·1)·diag(ω)·W·diag(ω)·(1·ω^T)`. Match.
